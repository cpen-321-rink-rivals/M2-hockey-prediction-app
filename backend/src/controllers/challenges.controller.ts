...
export class ChallengesController {
  ...
  async getUserChallenges(req: Request, res: Response) {
    try {
      ...
      const challenges = await challengeModel.getUserChallenges(req.user.id, status);
      ...
      const groupedChallenges = {
        ...
        cancelled: challenges.filter(c => c.status === ChallengeStatus.CANCELLED),
      };
  ...
  async joinChallenge(req: Request, res: Response) {
    try {
      ...
      const challenge = await challengeModel.joinChallenge(id, req.user.id, ticketId);

      if (!challenge) {
        return res.status(400).json({ 
          error: 'Bad Request',
          message: 'Unable to join challenge. It may be full, you may already be a member, or it may not exist.' 
        });
      }
  ...
  async leaveChallenge(req: Request, res: Response) {
    try {
      ...
      if (!challenge) {
        return res.status(400).json({ 
          error: 'Bad Request',
          message: 'Unable to leave challenge. You may not be a member, be the owner, or the challenge may be live/finished.' 
        });
      }
